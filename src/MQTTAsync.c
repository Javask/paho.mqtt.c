...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if (m->c->sslopts)
	{
		...
		{
			if(m->c->sslopts->ssl_engine)
				free((void*)m->c->sslopts->ssl_engine);
		}
	...
	if (options->struct_version != 0 && options->ssl)
	{
		...
		if(m->c->sslopts->struct_version >= 6)
		{
			...
			if(options->ssl->ssl_engine)
			{
				m->c->sslopts->ssl_engine = MQTTStrdup(options->ssl->ssl_engine);
			}
			else if(options->ssl->privateKeyFormat == MQTT_SSL_PRIVATE_KEY_FORMAT_ENGINE)
			{
				rc = MQTTASYNC_BAD_MQTT_OPTION;
				goto exit;
			}
#else
			if(options->ssl->ssl_engine ||
			   options->ssl->privateKeyFormat == MQTT_SSL_PRIVATE_KEY_FORMAT_ENGINE)
			{
				rc = MQTTASYNC_BAD_MQTT_OPTION;
				goto exit;
			}
#endif
			m->c->sslopts->privateKeyFormat = options->ssl->privateKeyFormat;
		}
	}
#else
	if (options->struct_version != 0 && options->ssl)
